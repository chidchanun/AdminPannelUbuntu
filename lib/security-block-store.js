...
import { blockIp, getThreatSnapshot, setThreatConfig, setWhitelist, unblockIp } from "@/lib/threat-guard";

const DEFAULT_BLOCK_STORE_PATH = "logs/security-blocks.json";
...
function getDefaultSettings() {
  return {
    ...
    whitelistIps: parseList(process.env.BOT_WHITELIST_IPS),
  };
}
...
function normalizeSettings(settings = {}) {
  ...
  const result = SETTING_KEYS.reduce((settingsResult, key) => {
    settingsResult[key] = typeof settings[key] === "boolean" ? settings[key] : defaults[key];

    return settingsResult;
  }, {});

  result.whitelistIps = normalizeWhitelist(settings.whitelistIps || defaults.whitelistIps);
  setWhitelist(result.whitelistIps);
...
export async function loadPersistentBlocks() {
  ...
  if (blocks.length !== stored.blocks.length) {
    await writeBlockFile({ blocks, settings });
  }
...
export async function updateSecuritySettings(updates) {
  ...
  if (Object.hasOwn(updates || {}, "whitelistIps")) {
    next.whitelistIps = normalizeWhitelist(updates.whitelistIps);
  }

  setWhitelist(next.whitelistIps);
...
export async function addWhitelistEntry(entry) {
  ...
  const next = normalizeWhitelist([...settings.whitelistIps, entry]);

  return updateSecuritySettings({ whitelistIps: next });
}

export async function removeWhitelistEntry(entry) {
  const settings = await getSecuritySettings();
  const target = String(entry || "").trim();
  const next = settings.whitelistIps.filter((item) => item !== target);
