...
function normalizeAlertSettings(settings = {}) {
  ...
  const discordUserIds = Array.isArray(settings.discordUserIds)
    ? settings.discordUserIds
    : parseList(settings.discordUserIds || process.env.DISCORD_USER_IDS);

  return {
    discordBotEnabled: typeof settings.discordBotEnabled === "boolean"
      ? settings.discordBotEnabled
      : false,
    discordBotToken: String(settings.discordBotToken || process.env.DISCORD_BOT_TOKEN || "").trim(),
    discordChannelId: String(
      settings.discordChannelId || process.env.DISCORD_CHANNEL_ID || "",
    ).trim(),
    discordUserIds: [...new Set(discordUserIds.map((id) => String(id).trim()).filter(Boolean))],
...
function normalizeAuditRetentionSettings(settings = {}) {
  return {
    maxAgeDays: parseNumber(settings.maxAgeDays, Number(process.env.AUDIT_RETENTION_DAYS || 30), {
      max: 3650,
      min: 1,
    }),
    maxEntries: parseNumber(
      settings.maxEntries,
      Number(process.env.AUDIT_RETENTION_MAX_ENTRIES || 10000),
      { max: 1000000, min: 100 },
    ),
  };
}

function normalizeTwoFactorSettings(settings = {}) {
  const users = settings.users && typeof settings.users === "object" ? settings.users : {};
  ...
  return {
    ...
    users: Object.fromEntries(
      Object.entries(users)
        ...
        .filter(([username, config]) => username && config.secret),
    ),
  };
}
...
export async function getAuditRetentionSettings() {
  ...
  return normalizeAuditRetentionSettings(stored.auditRetention);
}

export async function updateAuditRetentionSettings(updates) {
  const stored = await readSettingsFile();
  const auditRetention = normalizeAuditRetentionSettings({
    ...normalizeAuditRetentionSettings(stored.auditRetention),
    ...(updates || {}),
  });
...
export async function getTwoFactorSettings() {
  ...
  return normalizeTwoFactorSettings(stored.twoFactor);
}

export async function updateTwoFactorSettings(updates) {
  const stored = await readSettingsFile();
  const current = normalizeTwoFactorSettings(stored.twoFactor);
  const twoFactor = normalizeTwoFactorSettings({
    ...current,
    ...(updates || {}),
    users: {
      ...current.users,
...
export async function setUserTwoFactor(username, config) {
  const stored = await readSettingsFile();
  const current = normalizeTwoFactorSettings(stored.twoFactor);
  ...
  const twoFactor = normalizeTwoFactorSettings({
    enabled: Object.keys(users).length > 0,
    users,
  });
