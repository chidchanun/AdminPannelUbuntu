...
## 9. sudoers For Service, Firewall, Packages, And Updates
...
ChidchanunServer ALL=(root) NOPASSWD: /usr/bin/apt-get update
ChidchanunServer ALL=(root) NOPASSWD: /usr/bin/apt-get upgrade -y
ChidchanunServer ALL=(root) NOPASSWD: /usr/bin/apt-get install -y --no-install-recommends *
```
...
## 12. Alerts, Updates, And Versions
...
หน้า Updates ใช้ `apt list --upgradable` และ `/var/run/reboot-required.pkgs` เพื่อดู package update, security update, และ reboot required
