...
import { getSecurityTuningSettings } from "@/lib/admin-settings";
...
function analyzeConnections(connections, tuning) {
  ...
  const byIp = [...groups.values()]
    .map((group) => ({
      ...
      reasons: [
        group.total >= tuning.portConnectionLimit ? "high connection count" : null,
        group.ports.size >= tuning.portSpreadLimit ? "many target ports" : null,
        group.synReceived >= tuning.synRecvLimit ? "many half-open connections" : null,
      ].filter(Boolean),
    }))
...
async function getWebLogSignals(tuning) {
  if (process.platform !== "linux") {
    return {
      checked: false,
  ...
  return {
    ...
    suspicious: [...groups.values()]
      .filter((group) => group.scanHits >= tuning.webScanLimit)
      .map((group) => ({
        ...group,
...
async function getAuthSignals(tuning) {
  if (process.platform !== "linux") {
    return {
      checked: false,
  ...
  return {
    ...
    failedByIp: [...groups.values()]
      .filter((group) => group.total >= tuning.authFailureLimit)
...
export async function getServerSecuritySnapshot() {
  const tuning = await getSecurityTuningSettings();
  const [connections, webLogs, auth] = await Promise.all([
    getConnections(),
    getWebLogSignals(tuning),
    getAuthSignals(tuning),
  ]);
  const connectionAnalysis = analyzeConnections(connections.connections, tuning);
  ...
  return {
    ...
    thresholds: {
      authFailureLimit: tuning.authFailureLimit,
      botRateLimitPerMinute: tuning.botRateLimitPerMinute,
      botScanLimit: tuning.botScanLimit,
      portConnectionLimit: tuning.portConnectionLimit,
      portSpreadLimit: tuning.portSpreadLimit,
      synRecvLimit: tuning.synRecvLimit,
      webScanLimit: tuning.webScanLimit,
    },
    webLogs,
  };
}
